(function () {
  ...
  function render(profile) {
    ...
    const regTile = (label, value) => `<div class="kpi"><small>${label}</small><strong>${value === "Non renseigné" ? '<span class="data-missing">Non renseigné</span>' : value}</strong></div>`;
    const reglementationSection = !isEpci && !isDepartment ? section("07 · OUTILS FONCIERS", "Réglementation locale et positionnement SDRIF", `<div class="kpi-grid kpi-grid-six">${regTile("Droit de préemption urbain", tagVal(profile.reglementation?.dpu?.value, DPU_LABELS))}${regTile("Permis de louer", tagVal(profile.reglementation?.permis_louer?.value, ACTIF_LABELS))}${regTile("Permis de diviser", tagVal(profile.reglementation?.permis_diviser?.value, ACTIF_LABELS))}${regTile("Typologie SDRIF-E", profile.sdrif?.entite_geo?.label || "Non renseigné")}</div><p class="method-note-small">DPU et permis de diviser : sources en cours de consolidation par la DDT 95 — « Non renseigné » n’équivaut jamais à « aucun dispositif ». Permis de louer : Communauté d’Agglomération Plaine Vallée (data.gouv.fr) là où le dispositif est numérisé. Typologie SDRIF-E : Région Île-de-France (data.gouv.fr) — situe le rôle attendu de la commune dans l’armature urbaine régionale, la capacité résiduelle en hectares n’étant pas disponible en open data à ce jour.</p>`) : "";
    root.innerHTML = `<div id="report">
      ...
      <div class="report-body">
        ...
        ${reglementationSection}
        ${section("06 · SOURCES ET MÉTHODE", "Bien lire cette fiche", `<div class="method-note"><strong>Consommation d’espace :</strong> Cerema / DGALN, Fichiers fonciers, 2011-2024. <strong>Friches :</strong> Cartofriches, inventaire évolutif consulté en 2026. <strong>Forme résidentielle :</strong> Insee, Recensement 2023, logement. <strong>Permis de louer :</strong> CA Plaine Vallée (data.gouv.fr). <strong>Définitions :</strong> Portail national de l’artificialisation et IGN OCS GE.<br><br><strong>Méthode :</strong> le repère 2021-2030 correspond à 50 % de la consommation observée en 2011-2020. Il est présenté à des fins pédagogiques et ne remplace pas la territorialisation réglementaire. Une absence dans Cartofriches ne prouve pas l’absence de friche. DPU, permis de diviser et positionnement SDRIF sont en cours de consolidation par la DDT 95 : une absence de donnée n’équivaut jamais à une absence de dispositif. Licence Ouverte / Etalab.</div>`)}
      </div></div>`;
  }
  Promise.all([
