(function () {
  ...
  function render(profile) {
    ...
    const reglementationSection = !isEpci && !isDepartment ? section("07 · OUTILS FONCIERS", "Réglementation locale et positionnement SDRIF", `<div class="kpi-grid kpi-grid-six">${regTile("Droit de préemption urbain", tagVal(profile.reglementation?.dpu?.value, DPU_LABELS))}${regTile("Permis de louer", tagVal(profile.reglementation?.permis_louer?.value, ACTIF_LABELS))}${regTile("Permis de diviser", tagVal(profile.reglementation?.permis_diviser?.value, ACTIF_LABELS))}${regTile("Situation SDRIF", tagVal(profile.sdrif?.situation?.value, SDRIF_LABELS))}</div><p class="method-note-small">DPU, permis de diviser et positionnement SDRIF : sources en cours de consolidation par la DDT 95 — « Non renseigné » n’équivaut jamais à « aucun dispositif ». Permis de louer : Communauté d’Agglomération Plaine Vallée (data.gouv.fr) là où le dispositif est numérisé.</p>`) : "";
